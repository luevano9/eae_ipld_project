...
# ----- Page configs -----
st.set_page_config(
    page_title="Fernando's Portfolio",
    page_icon="📊",
)
...
no_director = movies_df["director"].isnull()
num_missing_directors = no_director.loc[movies_df["director"].isnull() == 1].count()

# TODO: Ex 2.4: How many different countries are there in the data?
movies_wo_country = movies_df["country"].fillna("Unknown").tolist()
l = ", ".join(movies_wo_country)
unique = l.split(", ")
set(unique)
n_countries = len(set(unique))

# TODO: Ex 2.5: How many characters long are on average the title names?
lenght = lambda x: len(x)
title_lenght = movies_df["title"].apply(lenght)
avg_title_length = title_lenght.mean().round(2)
...
movies_by_year = movies_df.loc[movies_df["release_year"] == year, ["country", "title"]].value_counts("country")
top_10_countries = movies_by_year.head(10)
...
new_df = movies_df[movies_df["type"] == "Movie" ]

num = lambda x: int(str(x).strip(" minutes"))

new_df["int"] = new_df["duration"].apply(num)

movies_avg_duration_per_year = new_df.groupby("release_year")["int"].mean().round(2)
...
if movies_avg_duration_per_year is not None:
    ...
    plt.plot(movies_avg_duration_per_year, color = "green", alpha = 0.7)
