...
temps_df["AvgTemperatureCelsius"] = (5 * (temps_df["AvgTemperatureFahrenheit"] - 32) / 9).round(2)       # uncomment this line to complete it
...
if unique_cities_list is not None and len(selected_cities) > 0:
    ...
    for city in selected_cities:
        ...
        plt.plot(city_df_period["Date"], city_df_period["AvgTemperatureCelsius"], alpha = 0.70)
 
    plt.title("Average Temperature")   # TODO
    plt.xlabel("Year")  # TODO
    ...
    for city in selected_cities:
        city_df = temps_df[temps_df["City"] == city]          
        city_df_period = city_df[(city_df["Date"] >= start_date) & (city_df["Date"] <= end_date)]     
        plt.hist(city_df_period["AvgTemperatureCelsius"], bins= 20, alpha = 0.40)                    
